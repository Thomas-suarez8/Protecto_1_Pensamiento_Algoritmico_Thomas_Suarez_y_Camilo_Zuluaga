#Modulo peliculas - Pensamiento Algoritmico
...
def encontrar_pelicula(nombre_pelicula: str, p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> dict:
    ...
    """
    nombre_lower = nombre_pelicula.strip().lower()

    if p1["nombre"].lower() == nombre_lower:
        return p1
    if p2["nombre"].lower() == nombre_lower:
        return p2
    if p3["nombre"].lower() == nombre_lower:
        return p3
    if p4["nombre"].lower() == nombre_lower:
        return p4
    if p5["nombre"].lower() == nombre_lower:
        return p5
...
def reagendar_pelicula(peli: dict, nueva_hora: int, nuevo_dia: str,
    ...
    if control_horario:
        genero_lower = peli["genero"].lower()

        if "documental" in genero_lower and nueva_hora >= 2200:
            print("Error: con control horario activo, los documentales no se pueden programar a las 22:00 o más tarde.")
            return False

        if "drama" in genero_lower and nuevo_dia == "Viernes":
            print("Error: con control horario activo, los dramas no se pueden programar los viernes.")
            return False

        if nuevo_dia in dias_entre_semana:
            if nueva_hora >= 2300:
                print("Error: entre semana no se pueden programar películas a las 11:00 p.m. o más tarde.")
                return False
            if nueva_hora < 600:
                print("Error: entre semana no se pueden programar películas antes de las 6:00 a.m.")
                return False

    nueva_inicio_min = _hhmm_a_minutos(nueva_hora)
    nueva_fin_min = nueva_inicio_min + peli["duracion"]

    otras = [p1, p2, p3, p4, p5]
    for otra in otras:
        if otra["nombre"] == peli["nombre"]:
            continue
        if otra["dia"] == nuevo_dia:
            otra_inicio_min = _hhmm_a_minutos(otra["hora"])
            otra_fin_min = otra_inicio_min + otra["duracion"]
            if nueva_inicio_min < otra_fin_min and nueva_fin_min > otra_inicio_min:
                print("Error: conflicto de horario con la pelicula '" + otra["nombre"] + "' el " + nuevo_dia + ".")
                return False
...
def decidir_invitar(peli: dict, edad_invitado: int, autorizacion_padres: bool) -> bool:
    ...
    if edad_invitado < 15 and "terror" in genero_lower:
        return False

    if edad_invitado <= 10 and "familiar" not in genero_lower:
        return False

    if clasificacion == "7+" and edad_invitado < 7:
        if "documental" in genero_lower:
            return True
...
def _hhmm_a_minutos(hora_hhmm: int) -> int:
    return (hora_hhmm // 100) * 60 + (hora_hhmm % 100)